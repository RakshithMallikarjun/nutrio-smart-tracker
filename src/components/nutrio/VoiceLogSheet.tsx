...
import { X, Mic, Loader2, Check, Trash2, Sparkles } from "lucide-react";
import { useServerFn } from "@tanstack/react-start";
import { parseFoodText } from "@/lib/ai-food.functions";
...
export function VoiceLogSheet({ open, onClose, defaultMeal, onAdd }: Props) {
      ...
      <div className="animate-fade-in flex max-h-[92vh] w-full max-w-md flex-col overflow-y-auto rounded-t-[2.5rem] bg-white p-6 pb-32" onClick={(e) => e.stopPropagation()}>
        ...
        {!supported ? (
          ...
          <>
            ...
            {!listening && !loading && transcript && items.length === 0 && (
              <button
                onClick={() => analyze(transcript)}
                className="mt-3 rounded-full bg-cream py-2 text-sm font-extrabold text-charcoal sage-border-soft"
              >
                Analyze transcript
              </button>
            )}
            ...
            {items.length > 0 && (
              <div className="mt-4 space-y-2">
                {items.map((it, idx) => (
                  <div key={it.key} className="flex items-center gap-3 rounded-2xl bg-white p-3 sage-border">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-[11px] font-bold" style={{ color: it.matched ? "#b7c6c2" : "#ca0013" }}>
                        {it.matched ? `${it.food.serving} · ${it.food.calories} kcal` : "Not in database — skipped"}
                      </p>
                    </div>
                    <button
